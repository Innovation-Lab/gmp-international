...
@section('content')
                        ...
                        <div class="l-grid__1">
                          <div class="l-grid__item">
                            <ul class="p-formList u-max--320">
                              ...
                              <li class="p-formList__item">
                                @forelse(data_get($fix_product, 'color.edit', []) as $key => $color)
                                  ...
                                  <div class="p-formList__content add_color" id="js_delete_{{ $loop->index + 1 }}">
                                    ...
                                    <div class="p-formList__data  u-max--360">
                                      <select name="color[edit][{{ $key }}][m_color_id]" class="select2">
                                        <option value="" hidden>選択してください</option>
                                        @foreach($colors as $k => $v)
                                          <option value="{{ $k }}" @if(old('m_color_id', data_get($color, 'm_color_id')) == $k ) selected @endif>{{ $v }}</option>
                                        @endforeach
                                      </select>
                                    </div>
                                    <div class="p-formList__data  u-max--360">
                                      <input type="hidden" name="color[edit][{{ $key }}][id]" value="{{ data_get($color, 'm_color_id') }}">
                                      <input type="text" name="color[edit][{{ $key }}][url]" placeholder="https://www.sample.page.com/airbuggy.png" value="{{ data_get($color, 'url') }}">
                                    </div>
                                    @if ($loop->index != 0)
                                      <div class="p-formList__btn" style="margin-left: auto;">
                                        <a class="c-textButton__icon c-textButton--gray delete" onclick="deleteExistingColorForm({{ $loop->index + 1 }}, {{ data_get($product, 'id') }}, {{  data_get($color, 'm_color_id') }})">
                                          削除
                                        </a>
                                      </div>
                                    @endif
                                ...
                                @foreach(data_get($fix_product, 'color.add', []) as $key => $color)
                                  <div class="p-formList__content add_color" id="js_delete_{{ $loop->index + count(data_get($fix_product, 'color.edit', [])) + 1 }}">
                                    @if ($loop->index == 0 && count(data_get($fix_product, 'color.edit', [])) <= 0)
                                      <div class="p-formList__label">
                                        カラー（メイン画像）
                                      </div>
                                    @else
                                    ...
                                    @endif
                                    <div class="p-formList__data  u-max--360">
                                      <select name="color[add][{{ $key }}][m_color_id]" class="select2" style="background-color: #e4d6d6">
                                        <option value="" hidden>選択してください</option>
                                        @foreach($colors as $k => $v)
                                          <option value="{{ $k }}" @if(old('m_color_id', data_get($color, 'm_color_id')) == $k ) selected @endif>{{ $v }}</option>
                                        @endforeach
                                    ...
                                    <div class="p-formList__data  u-max--360">
                                      <input type="hidden" name="color[add][{{ $key }}][id]" value="{{ data_get($color, 'm_color_id') }}">
                                      <input type="text" name="color[add][{{ $key }}][url]" placeholder="https://www.sample.page.com/airbuggy.png" value="{{ data_get($color, 'url') }}" style="background-color: #e4d6d6">
                                    </div>
                                    @if ($loop->index == 0 && count(data_get($fix_product, 'color.edit', [])) <= 0)
                                    @else
                                      <div class="p-formList__btn" style="margin-left: auto;">
                                        <a class="c-textButton__icon c-textButton--gray delete" onclick="deleteColorForm({{ $loop->index + count(data_get($fix_product, 'color.edit', [])) + 1 }})">
                                          削除
                                        </a>
                                      </div>
                                    @endif
  ...
  <script>
      $(document).ready(function() {
          var num = {{ count(data_get($fix_product, 'color.add', [])) + count(data_get($fix_product, 'color.edit', [])) + 1}};
